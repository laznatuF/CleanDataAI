...
# [CORRECCIÓN 1] Importamos el cerebro semántico para detección inteligente
...
# [CORRECCIÓN 2] Desactivamos el modo seguro para usar tu IA real
...
try:
    if not USE_AUTOSPEC_FALLBACK:
        # [NOTA CRÍTICA] Asegúrate de renombrar 'autospect.py' a 'autospec.py' en tu carpeta application
        from app.application.autospec import auto_dashboard_spec  # type: ignore
    else:
        raise ImportError()
except Exception:
    # ---------- AUTOSPEC ROBUSTO INTEGRADO (Fallback) ----------
    def auto_dashboard_spec(
        ...
        Se usa solo si falla la carga del autospec principal.
        """
        import re
        ...
        priority = ["categoria", "category", "producto", "product", "cliente", "customer", "usuario", "user", "ciudad", "city", "region", "pais", "country", "metodo_pago", "medio_pago", "payment", "pago", "estado", "estatus", "status", "prioridad", "gerente", "canal", "tipo", "vendedor", "seller"]
        ...
        if primary_date:
            ...
            charts.append({
                ...
                "encoding": {
                    ...
                    "y": ({"field": primary_metric, "aggregate": "sum"} if primary_metric else {"field": primary_date, "aggregate": "count"}),
                },
            })
        ...
        if dims:
            ...
            charts.append({
                ...
                "encoding": {
                    "x": {"field": d0},
                    "y": ({"field": primary_metric, "aggregate": "sum"} if primary_metric else {"field": "__row__", "aggregate": "count"}),
                },
            })
        ...
        if primary_date and len(dims) >= 2:
            ...
            charts.append({
                ...
                "encoding": {
                    "x": {"field": primary_date, "timeUnit": "month"},
                    "y": {"field": d1},
                    "value": ({"field": primary_metric, "aggregate": "sum"} if primary_metric else {"field": "__row__", "aggregate": "count"}),
                },
            })
        elif dims:
            d0 = dims[0]
...
try:
    ...
    def build_generic_spec(df: pd.DataFrame, roles: Dict[str, str], title: str = "Dashboard") -> Dict[str, Any]:
        """Fallback de layout seguro."""
        first = df.columns[0]
        charts = [
            {"id": "safe_count", "type": "bar", "title": f"Conteo por {first}", "encoding": {"x": {"field": first}, "y": {"aggregate": "count"}}, "limit": 12},
            {"id": "safe_nulls", "type": "bar", "title": "Porcentaje de nulos por columna", "encoding": {"x": {"field": "__column__"}, "y": {"field": "__null_pct__"}}},
        ]
        return {
            "title": title,
...
def _stage(proc_id: str, stage: str):
    ...
    class _Ctx:
        ...
        def __exit__(self_inner, exc_type, exc, tb):
            ...
            if exc:
                append_history(proc_id, {"type": "stage_end", "stage": stage, "status": "failed", "duration_ms": dur_ms, "error": str(exc)})
            else:
...
def process_pipeline(proc_id: str) -> None:
    ...
    try:
        ...
        # 3) Inferencia de tipos (CON SEMANTICS)
        with _stage(proc_id, "InferenciaTipos"):
            # [CORRECCIÓN 3] Aquí activamos el cerebro semántico
            semantic_schema = infer_semantics(df)
            roles = semantic_schema.roles

            # Mantenemos la prioridad de las fechas que ya parseamos
        ...
        with _stage(proc_id, "Perfilado"):
            ...
            try:
                ...
                append_history(proc_id, {"type": "perfilado_export_error", "error": str(e)})
        ...
        with _stage(proc_id, "Dashboard"):
            # 6.a) Generar SPEC automático
            # Esto llamará a 'autospec.py' usando los roles inteligentes de Semantics.
        ...
        if not status:
            status = {
                "id": proc_id, "status": "failed", "progress": 0,
